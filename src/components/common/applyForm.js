...
const RefferenceNumber = ({ courseApplied, levelApplied }) => {
    ...
    axios
      ...
      .then((response) => {
        ...
        if (error) {
          ...
          setTimeout(setResponseResult, 20000, "");
          setLoading(false);
        }
      })
      .catch((error) => {
  ...
  let referenceCodeCheck = enableReference ? referenceCode : true;
  ...
  levelApplied =
    levelApplied.split("   ").length > 1
      ? levelApplied.split("   ").join(" + ")
      : levelApplied;
    ...
    <React.Fragment>
      {responseResult == "Success" && (
        <Notify style={{ background: "#01937c", marginTop: 30 }}>
          {" "}
          Your application has been submitted successfully. We will return you
          as soon as possible. Please keep checking your emails spam folder.{" "}
        </Notify>
      )}
      {responseResult == "Error" && (
        <Notify style={{ background: "#ff3333", marginTop: 30 }}>
          {" "}
          Server Error. Your application hasn't been submitted{" "}
        </Notify>
      )}
      <GeneralWrapper>
        <ClassicText>
          {courseApplied} {levelApplied == "IGCSE" ? null : levelApplied}
        </ClassicText>
        <Description>Scholarship and Pre-registration Form</Description>
        <FormBox onSubmit={Submit}>
          ...
          <SubmitButton disabled={!result || loading || !referenceCodeCheck}>
            {loading ? (
